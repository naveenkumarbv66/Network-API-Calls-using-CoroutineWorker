...
class CreatePostWorker(appContext: Context, params: WorkerParameters) : CoroutineWorker(appContext, params) {
    override suspend fun doWork(): Result {
        val title = inputData.getString("title") ?: "Demo Title"
        val body = inputData.getString("body") ?: "Demo Body"

        val repo = NetworkRepository(RetrofitClient.apiService)
        val result = repo.createDemoPost(title, body)
        return result.fold(
            onSuccess = { post ->
